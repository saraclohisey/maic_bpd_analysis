...
Optionally, these can be removed from output lists if desired.
...
formats:
    ...
    Ensembl gene/transcript ID (ENSG / ENST) (for human/mouse/rat/cat/horse so far)
...
Multispecies option added 04.12.24:
If option selected, required an additional 5th line (after NAMED_GENES)
to specify species of each list. Then if genes are ambiguous according to species
of origin, can select the appropriate homolog.
Options/species currently supported (have to be entered in exact form below):
human, mouse, rat, ferret, macaque, African green monkey, bovine, pig, chicken, horse,
dog, cat, sheep, goat
(Will remove this extra line from MAIC output to avoid confusing MAIC)
...
class ListFormatter(object):
    ...
    def __init__(self, options, singlefile=False):
        ...
        self.allowed_species = ['human', 'mouse', 'rat', 'ferret',
                                'macaque', 'African green monkey',
                                'bovine', 'pig', 'chicken', 'horse',
                                'dog', 'cat', 'sheep', 'goat']
...
class GeneList(object):
    ...
    def __init__(self, genelist, formatter, filename):
        ...
        if self.formatter.options.sp:
            self.species = genelist[4]
            self.firstgene = genelist[5]
        else:
            self.species = None
            self.firstgene = genelist[4]
        if self.formatter.options.gmt:
            self.genes = [i for i in genelist[1:] if i != ""]
        elif self.formatter.options.sp:
            self.genes = [i for i in genelist[5:] if i != ""]
        else:
    ...
    def check_fields(self):
        ...
        if self.formatter.options.sp:
            if self.species not in self.formatter.allowed_species:
                raise Exception('Valid species not found in line 5 of list {},' 
                         ' species must be one of {}.'.format(self.filename,
                                     self.formatter.allowed_species))
        else:
            if self.firstgene in self.formatter.allowed_species:
                raise Exception('Species name found in line 5 of list {}, did '
                                'you mean to specify -sp option?'.format(self.filename))
    ...
    def correct_categories(self):
        ...
        if 'screen' in self.category.lower() and ('crispr' not in self.category.lower() and 'rnai' not in self.category.lower()):
        ...
        for gene in corrected_list:
            ...
            for variant in [gene,
                ...
                try:
                    ...
                    continue
            if not found:
                if not self.formatter.options.x:
                    newlist.append(gene)
                self.unconverted.append(gene)
                continue
            
            # Adjust for species if option specified:
            # If human, remove all homolog entries
            # If not, select specific homolog if included in list
            if self.formatter.options.sp:
                if self.species == 'human':
                    genedata = {k:v for k,v in genedata.items()
                                if not k.startswith('homolog')}
                elif 'homolog_{}'.format(self.species) in genedata.keys():
                    genedata = {k:v for k,v in genedata.items()
                                if k == 'homolog_{}'.format(self.species)}
                else:
                    pass
                    # No species adjustment possible
            ...
            if len(genedata) == 1:
                ...
                # Ambiguous even after species adjustments.
                # leave tag in place for now to preserve rankings,
...
def get_parsed_options(args=None):
    ...
    parser.add_argument('-sp', '--sp', default=False, action='store_true',
                        help='species specification included in list headers')
    parser.add_argument('-x', '--x', default=False, action='store_true',
                        help='remove unconverted genes from output')
